          ...
          <ul class="p-0 m-0">
            @forelse($latestRequest as $req)
              <li class="d-flex mb-4 align-items-center pb-2">
                ...
                <div class="d-flex w-100 flex-wrap align-items-center justify-content-between gap-2">
                  <div class="me-2">
                    <h6 class="mb-0">{{ $req->user->name ?? $req->guest->name ?? 'Guest' }}</h6>

                    @foreach($req->cartItems as $cartItem)
                      <p class="mb-0">{{ $cartItem->item->name }} (Qty: {{ $cartItem->quantity }})</p>
                    @endforeach
                  </div>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  window.updateChart = function (range) {
    // AJAX request ke Laravel route sesuai range, atau ganti data dummy dulu
